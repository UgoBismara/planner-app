...
//   Boeuf haché 15 % MG   : 196 kcal  G:0   L:13   P:19
//   Agneau haché          : 260 kcal  G:0   L:20   P:18
//   Boeuf à braiser        : 150 kcal  G:0   L:6    P:22
...
//
// Oeufs & Lait
//   Oeuf entier (60 g/u)   : 155 kcal  G:1.1 L:11   P:13  (pour 100 g)
...
export const RECIPE_MACROS = {
  ...
  // 26. Boeuf bourguignon (×6 → /6) : 167g bœuf G0+L10+P36.7 · 33g lardons G0.2+L7.9+P5.3 · légumes G8 · 125ml vin G2.5
  ...
  // 50. Oeufs brouillés crémeux : 3 œufs G1.9+L19.8+P23.4 · 2cs crème G0.7+L5.6+P0.5 · 10g beurre G0+L8.1+P0 · 2 tranches pain de mie G23+L1.6+P3.8
...
export function extractFoodName(ingredient) {
  ...
  s = s.replace(
    /^[\d,./\s]+\s*(g|kg|ml|l|cl|c\.s\.|c\.c\.|boîtes?|sachets?|tranches?|gousses?|pincées?|filets?|feuilles?|branches?)?\s*(de\s+|d['']\s*|du\s+|des\s+|à\s+la\s+|au\s+)?/i,
    "",
  );
  s = s.replace(/\s*\(.*?\)/g, "");
  s = s.replace(
    /\s+(râpé[e]?s?|frais|fraîche[s]?|surgelé[e]?s?|cuit[e]?s?|fumé[e]?s?|haché[e]?s?|mûr[e]?|dénoyauté[e]?s?|concassé[e]?s?|entier[e]?s?|en\s+poudre|moulu[e]?s?).*/,
    "",
  );
